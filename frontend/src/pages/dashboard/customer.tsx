...
export default function Dashboard() {
  ...
  const [currentView, setCurrentView] = useState<"dashboard" | "explore">(
    "dashboard"
  );

  const [activeTab, setActiveTab] = useState<
    "hired" | "favorites" | "reviews"
  >("hired");

  // --- BUSINESS LOGIC & DATA STATES ---
  const [hiredWorkers, setHiredWorkers] = useState<Provider[]>([
    {
      id: 101,
      name: "Ramesh Thapa",
      phone: "9812345678",
      location: "Patan, Lalitpur",
      skill: "Senior Plumber",
      category: "Plumbing",
      avatar:
      ...
      jobsDone: 142,
      verified: true,
      bio: "Expert plumber with over 8 years of experience fixing leaks, pipe installation, and bathroom fixtures in Lalitpur area.",
    },
    {
      id: 102,
      name: "Suman Shrestha",
      ...
      jobsDone: 98,
      verified: true,
      bio: "Certified electrical systems technician specializing in house rewiring, generator repairs, and smart lighting installation.",
    },
  ]);

  const [favoriteWorkers, setFavoriteWorkers] = useState<Provider[]>([
    {
      id: 103,
      name: "Bijay Maharjan",
      phone: "9834567890",
      ...
      verified: true,
      bio: "Custom woodwork and furniture repairing specialist.",
    },
  ]);
  ...
  const [activeReviews, setActiveReviews] = useState<Review[]>([
    {
      id: 1,
      workerName: "Ramesh Thapa",
      rating: 5,
      comment:
        "Did an exceptional plumbing fix in our kitchen. Highly professional!",
      date: "2 days ago",
    },
  ]);
  ...
  const [availableProviders, setAvailableProviders] = useState<Provider[]>([
    ...
    },
    {
      ...
      verified: true,
      bio: "Deep cleaning specialist for houses and office premises.",
    },
    {
      id: 105,
      name: "Deepak K.C.",
      ...
      verified: false,
      bio: "Professional interior and exterior house painter.",
    },
  ]);

  // --- MODALS & INTERACTIONS STATE ---
  const [isBookingModalOpen, setIsBookingModalOpen] = useState(false);
  const [isProfileDetailModalOpen, setIsProfileDetailModalOpen] =
    useState(false);
  ...
  const [activeChatWorker, setActiveChatWorker] =
    useState<Provider | null>(null);

  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      sender: "worker",
      text: "Namaste! How can I help you with your maintenance needs today?",
    },
  ]);
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      ...
      message:
        "Suman Shrestha accepted your electrical maintenance request.",
      time: "10m ago",
      read: false,
    },
    {
      ...
      message: "Payment of Rs. 1,500 completed for Ramesh Thapa.",
      time: "1d ago",
      read: true,
    },
  ]);
  ...
  const handleSaveProfile = (
    ...
    if (
      tempPhone &&
      !/^(98|97)\d{8}$/.test(tempPhone)
    ) {
      setPhoneError(
        "Please enter a valid Nepalese mobile number (e.g., 98xxxxxxxx)"
      );
      return;
    }
  ...
  const handleLogout = () => {
    if (
      window.confirm(
        "Are you sure you want to log out of Kamdar Nepal?"
      )
    ) {
  ...
  // --- PROVIDER DETAIL ---
  const handleOpenProviderDetail = (
    provider: Provider
  ) => {
  ...
  const handleConfirmBooking = (
    ...
    setAvailableProviders((prev) =>
      prev.map((p) =>
        p.id === provider.id
          ? {
              ...p,
              jobsDone: p.jobsDone + 1,
            }
          : p
      )
    );

    const newHired: Provider = {
      id: provider.id || Date.now(),
      name: provider.name,
      phone: provider.phone,
      location: provider.location,
      skill:
        (provider.category || provider.skill || "General") +
        " Expert",
    ...
    setHiredWorkers((prev) => [
      newHired,
      ...prev.filter((worker) => worker.id !== newHired.id),
    ]);
    ...
    triggerToast(
      `Successfully dispatched booking request to ${provider.name}!`
    );
  ...
  const handleToggleFavorite = (
    ...
    setFavoriteWorkers((prev) => {
      ...
      if (exists) {
        triggerToast(
          `Removed ${provider.name} from favorites.`
        );
      ...
      triggerToast(
        `Added ${provider.name} to favorites!`
      );
  ...
  // --- CHAT ---
  const handleOpenChat = (worker: Provider) => {
    setActiveChatWorker(worker);

    setChatMessages([
      {
        sender: "worker",
        text: `Namaste! This is ${worker.name}. How can I assist you?`,
      },
    ]);
  ...
  const handleSendMessage = (
    ...
    window.setTimeout(() => {
      setChatMessages((prev) => [
        ...
        {
          sender: "worker",
          text: `Thanks for your message regarding "${trimmedMessage}". I will check my schedule and update you shortly!`,
        },
      ]);
    }, 1000);
  };
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(
                      (notification) => (
                        <div
                          key={notification.id}
                          className="p-3 rounded-xl bg-slate-800/50 border border-slate-800 space-y-1"
                        >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      ...
                      and service location so artisans
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
                <div className="flex items-center space-x-5">
                  <div className="relative">
                    ...
                    {user.isVerified && (
                      <span
                        className="absolute -bottom-2 -right-2 bg-blue-600 text-white p-1.5 rounded-full text-xs shadow-md"
                  ...
                  {hiredWorkers.length === 0 ? (
                    <div className="col-span-2 text-center py-12 bg-slate-900/40 border border-slate-800 rounded-3xl">
                      ...
                      <button
                        ...
                        className="mt-3 text-xs bg-blue-600 text-white px-4 py-2 rounded-xl font-semibold"
                      >
                        Explore Workers
                      </button>
                    </div>
                    ...
                    hiredWorkers.map(
                      (worker) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-center space-x-4">
                              <img
                                src={worker.avatar}
                                alt={worker.name}
                                className="w-14 h-14 rounded-2xl object-cover border border-slate-700 group-hover:scale-105 transition-transform"
                              />
                              ...
                              <div>
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-[10px] text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-md font-medium">
                                    View Profile
                                  </span>
                                </div>
                          ...
                          <div
                            className="flex items-center justify-between pt-3 border-t border-slate-800/80 text-xs"
                            onClick={(e) =>
                              e.stopPropagation()
                            }
                          >
                  ...
                  {favoriteWorkers.length === 0 ? (
                    ...
                    favoriteWorkers.map(
                      (fav) => (
                        <div
                          key={fav.id}
                          className="bg-slate-900/80 border border-slate-800/80 rounded-3xl p-5 flex items-center justify-between shadow-xl"
                        >
                          <div
                  ...
                  {activeReviews.length === 0 ? (
                    ...
                    activeReviews.map(
                      (review) => (
                        <div
                          key={review.id}
                          className="bg-slate-900/80 border border-slate-800/80 rounded-3xl p-5 space-y-3 shadow-xl"
                        >
                          <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-4">
                            <div className="relative">
                              <img
                                ...
                                className="w-16 h-16 rounded-2xl object-cover border border-slate-700 group-hover:scale-105 transition-transform"
                              />

                              {provider.verified && (
                                <span
                                  className="absolute -bottom-1 -right-1 bg-blue-600 text-white p-1 rounded-full text-[10px]"
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed bg-slate-950/60 p-4 rounded-2xl border border-slate-800">
                    {selectedProvider.bio ||
                      "Professional handyman providing top-quality repair and setup services across Lalitpur and Kathmandu area."}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="relative flex items-center">
                    <input
                      ...
                      className="w-full bg-[#05070c] border border-slate-700 rounded-2xl pl-4 pr-12 py-3.5 text-xs text-white focus:outline-none focus:border-blue-500 transition-all cursor-pointer"
                    />
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-end sm:justify-center p-0 sm:p-4">
          <div className="bg-slate-900 border border-slate-800 w-full sm:max-w-md h-[550px] rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-800 bg-slate-950 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <img
                  src={activeChatWorker.avatar}
                  alt={activeChatWorker.name}
